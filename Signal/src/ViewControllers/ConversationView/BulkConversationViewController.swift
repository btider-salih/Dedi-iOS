...
class BulkConversationViewController: UIViewController {
    ...
    func setupBannerView(){
        ...
        if #available(iOS 11.0, *) {
            bannerHeaderLabel.topAnchor.constraint(equalTo: view.safeAreaLayoutGuide.topAnchor, constant: 30).isActive = true
        } else {
            bannerHeaderLabel.topAnchor.constraint(equalTo: view.topAnchor, constant: 30).isActive = true
        }
...
//        guard let sender = self.messageSender else{ return }
        ...
        ThreadUtil.enqueueMessage(withContactShare: contact, in: contactThread)
//        ThreadUtil.enqueueMessage(with: contact, in: contactThread) { (error) in
//            if let err = error{
...
//        }
    ...
    func sendAttachmentMessage(to recipientId:String, with attachment:SignalAttachment){
        guard let sender = self.messageSender else{ return }
        let contactThread = TSContactThread.getOrCreateThread(contactId: recipientId)
        ThreadUtil.sendMessageNonDurably(with: [attachment], in: contactThread, messageBody: nil, quotedReplyModel: nil, messageSender: sender) { (error) in
            if let err = error{
                print(err.localizedDescription)
                return
            }
    ...
    func sendRegularTextMessage(to recipientId:String, with messageBody:String){
        guard let sender = self.messageSender else{ return }
        let contactThread = TSContactThread.getOrCreateThread(contactId: recipientId)
        ThreadUtil.sendMessageNonDurably(withText: messageBody, in: contactThread, quotedReplyModel: nil, messageSender: sender, success: {
...
//        actionSheet.addAction(chooseDocumentAction)
    ...
    func sendQualityAdjustedAttachment(for videoUrl:URL, and filename:String?){
        ...
        guard let dataSource = DataSourcePath.dataSource(with: videoUrl, shouldDeleteOnDeallocation: true) else{
        ...
        compressionResult.0.done { (attachment) -> Void in
            AssertIsOnMainThread(file: #function)
