...
- (void)checkForLatestLocationSentDate{
    ...
    if (latestLocationSent){
        ...
        if (fabs([now timeIntervalSinceDate:latestLocationSent]) > 3){
            ...
            [controller addAction: [UIAlertAction actionWithTitle:NSLocalizedString(@"LOCATION_SEND_FREQUENCY_WARNING_TITLE", @"Warning title for location sending interval (3 seconds)")
                                                            style:UIAlertActionStyleDefault
                                                          handler:nil]];
